...
program
    ...
    .on('--help', () => {
        ...
        console.log('    $ dwui new page')
        console.log()
        console.log(chalk.gray('    # new a component'))
        console.log('    $ dwui new component')
        console.log()
    })
    .on('exit',()=>{
        console.log()
...
if(newType!=='component'&&newType!=='page'){
    console.log()
    console.log(chalk.red(`不支持当前类型 <${newType}>, 请在 component 和 page 之前选择`))
    console.log()
    process.exit()
}
//检测版本，并执行主函数
...
function main(){
    //获取dwui.config.js的位置
    let configPath=path.join(process.cwd(),'dwui.config.js')
    if(program.config){
        configPath=path.isAbsolute(program.config)?program.config:path.join(process.cwd(),program.config)
    }
    const dwuiConfig = require(configPath)
    const templates = dwuiConfig[`${newType}`].templates
    const defaultDest = dwuiConfig[`${newType}`].output
    const helpers = dwuiConfig.helpers
    //注册helpers
